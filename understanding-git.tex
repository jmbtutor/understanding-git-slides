...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Commits are identified by a SHA-1 hash. This hash can be abbreviated to its unique prefix (minimum of 4 characters).
...
\begin{frame}
  ...
  \goal{Reset \gbranch{topic} to commit \grefspec{B} without discarding changes.}
...
\begin{frame}
  \frametitle{\gitcmd{push -f}}
  \framesubtitle{Push new commits upstream and forcibly update the tracking branch reference.}
  ...
  \gremotebranch{origin/topic} was forcibly updated to point to the tip of \gbranch{topic} (\grefspec{E$'$}).
