...
\begin{frame}
  ...
  \goal{Without moving \gHEAD{}, create a new branch called \gbranch{feature} on the current commit.}

  \defaultrepo
\end{frame}

\begin{frame}
  \frametitle{\gitcmd{branch feature}}
  \framesubtitle{Create branch \gbranch{feature} on the current commit.}
  ...
  A branch called \gbranch{feature} is created on the current commit and \gHEAD{} is not moved.
\end{frame}

\begin{frame}
  \frametitle{\gitcmd{branch -d feature}}
  \framesubtitle{Delete the branch \gbranch{feature}.}
  ...
  The branch \gbranch{feature} has been deleted.
\end{frame}
...
\begin{frame}
  ...
  \end{itemize}
  \vfill
  \begin{block}{\gitcmd{checkout -b feature}}
    The \gflag{-b} flag is transactionally equivalent to doing a\\
    \hspace{2em}\gitcmd{branch feature}\\
    followed by a\\
    \hspace{2em}\gitcmd{checkout feature}.
  \end{block}
...
\begin{frame}
  \frametitle{\gitcmd{commit}}
  \framesubtitle{Make a new commit \grefspec{F} off of commit \grefspec{B}.}
  \begin{figure}
    \centering
    \begin{tikzpicture}
      \gitDAG{
...
\begin{frame}
  \frametitle{\texttt{\gitcmd{merge --squash topic} \&\& \gitcmd{commit}}}
